-- =============================================================================
-- 0039_feed_no_deadend.sql  ·  Der Feed endet nicht mehr in einer Sackgasse
--
-- Der Fehler
-- ----------
-- "Man braucht irgendwie einen Seitenreload, um weiterscrollen zu koennen."
--
-- Genau so ist es, und der Grund ist simpel: get_feed wusste nicht, was
-- der Nutzer GERADE AUF DEM SCHIRM hat. Waren alle Karten einmal gelesen,
-- lieferte es dieselben zurueck, die schon in der Liste standen. Die App
-- wirft Dubletten weg (appendArranged), also kam nichts an, die Liste
-- wuchs nicht - und wer unten steht, steht fest.
--
-- Nach einem Neuladen ist die Liste leer, dieselben Karten sind wieder
-- "neu", und es geht weiter. Deshalb half der Reload.
--
-- Die Behebung
-- ------------
-- Die App schickt mit, was sie schon hat. Was dort steht, kommt nicht
-- noch einmal.
--
-- Und die Reihenfolge der Stufen bleibt, aber mit einem Notausgang:
--
--   1. Ungelesenes, so viel wie moeglich.
--   2. Hoechstens jede zehnte Karte eine Wiederholung - solange es
--      ueberhaupt Ungelesenes gibt.
--   3. Gibt es NICHTS Ungelesenes mehr, wird der Stapel mit
--      Wiederholungen gefuellt. Das ist der Notausgang: ein Feed, der
--      aufhoert, ist schlimmer als einer, der sich wiederholt - und man
--      hat dann wirklich alles gelesen.
--
-- Stufe 3 gab es vorher immer, deshalb fiel die Sackgasse nicht auf; seit
-- 0035 greift sie nur noch im Notfall, und genau dadurch wurde der alte
-- Fehler sichtbar.
-- =============================================================================

create or replace function public.get_feed(
  p_batch_size int default 10,
  -- Was die App schon geladen hat. Ohne diese Liste kann der Server nicht
  -- wissen, dass seine Antwort beim Empfaenger im Papierkorb landet.
  p_exclude uuid[] default '{}'
)
returns setof public.content_items
language plpgsql security definer set search_path = ''
as $fn$
declare
  v_user    uuid := auth.uid();
  v_p       public.profiles;
  v_langs   text[];
  v_n_news  int;
  v_n_know  int;
  v_repeats int;
  v_fresh   int;
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  select * into v_p from public.profiles where id = v_user;

  v_langs := public.user_feed_languages(v_user);
  v_n_news := greatest(1, (p_batch_size * 4) / 10);
  v_n_know := greatest(1, p_batch_size - v_n_news);
  v_repeats := greatest(1, p_batch_size / 10);

  return query
  with interests as (
    select uc.category_id, uc.interest_weight, uc.difficulty_pref
      from public.user_categories uc
     where uc.user_id = v_user
  ),
  seen as (
    select ucs.content_id, ucs.last_seen_at
      from public.user_content_state ucs
     where ucs.user_id = v_user
  ),
  pool as (
    select ci.id                  as content_id,
           ci.content_type        as content_type,
           (s2.content_id is not null)     as already_seen,
           s2.last_seen_at,
           coalesce(i.interest_weight, 0.3)
           * exp(-extract(epoch from (now() - coalesce(ci.published_at, ci.created_at)))
                 / 172800.0)
           * (1.0 - abs(ci.difficulty - coalesce(i.difficulty_pref, 2)) * 0.18)
           * (coalesce(s.trust_score, 50) / 100.0)
           * public.language_weight(ci.language, v_p.feed_english_pct)
           * (0.85 + random() * 0.3)
             as score
      from public.content_items ci
      join public.categories cat on cat.id = ci.primary_category_id
      left join lateral (
        select ui.interest_weight, ui.difficulty_pref
          from interests ui
         where ui.category_id = ci.primary_category_id
            or ui.category_id = cat.parent_id
         order by (ui.category_id = ci.primary_category_id) desc
         limit 1
      ) i on true
      left join public.sources s on s.id = ci.primary_source_id
      left join seen s2 on s2.content_id = ci.id
     where ci.status = 'approved'
       and ci.language = any(v_langs)
       and ci.content_type <> 'course_lesson'
       and (ci.expires_at is null or ci.expires_at > now())
       and (ci.region_code is null or ci.region_code = v_p.country_code
            or ci.region_code = v_p.region_code)
       -- Der Kern der Behebung.
       and not (ci.id = any(p_exclude))
  ),
  fresh as (select * from pool where not already_seen),
  picked as (
    (select content_id from fresh where content_type = 'news'
      order by score desc limit v_n_news)
    union
    (select content_id from fresh where content_type in ('knowledge','interactive')
      order by score desc limit v_n_know)
  ),
  repeats as (
    select content_id
      from pool
     where already_seen
       and content_id not in (select content_id from picked)
     order by last_seen_at asc nulls first
     -- Der Notausgang: ohne frisches Material darf der ganze Stapel aus
     -- Wiederholungen bestehen.
     limit case when (select count(*) from picked) = 0 then p_batch_size else v_repeats end
  )
  select ci.*
    from (
      select content_id, 0 as tier, random() as r from picked
      union all
      select content_id, 1 as tier, random() as r from repeats
    ) f
    join public.content_items ci on ci.id = f.content_id
   order by f.tier, f.r
   limit p_batch_size;
end
$fn$;

grant execute on function public.get_feed(int, uuid[]) to authenticated;

-- Die alte Signatur bleibt bestehen, damit ein noch nicht aktualisierter
-- App-Stand nicht abstuerzt.
create or replace function public.get_feed(p_batch_size int default 10)
returns setof public.content_items
language sql security definer set search_path = ''
as $fn$
  select * from public.get_feed(p_batch_size, '{}'::uuid[]);
$fn$;

grant execute on function public.get_feed(int) to authenticated;
