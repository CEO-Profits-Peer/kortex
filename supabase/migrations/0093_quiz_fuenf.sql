-- =============================================================================
-- 0093_quiz_fuenf.sql  ·  Quiz mit PRO bis zu fuenf Antworten (vorher vier)
--
-- Kostenlos bleiben es drei. Nur diese Grenze aendert sich; create_post ist
-- vollstaendig aus 0091 uebernommen (Regel: nie aus einem Ausschnitt neu
-- schreiben). post_quiz_loesung speichert den Index der richtigen Antwort
-- ohne Obergrenze, braucht also keine Aenderung.
-- =============================================================================
...
-- --- Selbsttest: Quiz mit fuenf Antworten als PRO, mit Rollback ------------------------
do $test$
declare
  v_id uuid;
  v_j  jsonb;
begin
  ...
  if v_id is null then raise notice 'Selbsttest 0093: kein Konto'; return; end if;
  perform set_config('request.jwt.claims', json_build_object('sub', v_id, 'role', 'authenticated')::text, true);
  begin
    update public.profiles set plan = 'gifted', plan_expires_at = now() + interval '1 day' where id = v_id;
    v_j := public.create_post('Selbsttest Quiz', 'quiz', null, null,
                              '{"optionen": ["a1", "b2", "c3", "d4", "e5"], "richtig": 4}'::jsonb);
    if v_j->>'id' is null then raise exception 'Selbsttest 0093: kein Beitrag: %', v_j; end if;
    raise exception 'selbsttest_ok';
  exception when others then
    if sqlerrm <> 'selbsttest_ok' then raise; end if;
  end;
  raise notice 'Selbsttest 0093: ok';
end
$test$;
